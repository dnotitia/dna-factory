...
class DnotitiaSFTTrainer(SFTTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        for i in range(len(inputs['input_ids'])):
            if self._debug_count < self.debug_first_n_batches and 'input_ids' in inputs and 'labels' in inputs:
                ...
                for i, (input_id, label_id) in enumerate(zip(input_ids, labels)):
                    ...
                    token_text = self.processing_class.decode([input_id], skip_special_tokens=False, errors='replace')
