...
def validate_messages_field(messages, row_idx, field_name):
    ...
    if not isinstance(messages, list) or len(messages) == 0:
        print(
            f"{YELLOW}Messages data:{RESET} {json.dumps(messages, indent=2, ensure_ascii=False)}"
        )
    ...
    for msg_idx, message in enumerate(messages):
        if not isinstance(message, dict):
            ...
            return (
                False,
                f"Row {row_idx}, Field '{field_name}', Message {msg_idx}: Not a dictionary",
            )
        ...
        if "content" not in message or "role" not in message:
            ...
            return (
                False,
                f"Row {row_idx}, Field '{field_name}', Message {msg_idx}: Missing 'content' or 'role'",
            )
        ...
        if role not in ["user", "assistant"]:
            ...
            return (
                False,
                f"Row {row_idx}, Field '{field_name}', Message {msg_idx}: Invalid role '{role}' (must be 'user' or 'assistant')",
            )
...
def validate_dataset(dataset):
    ...
    for idx, example in enumerate(dataset):
        ...
        if not is_valid:
            print(
                f"{YELLOW}Complete data:{RESET} {json.dumps(example, indent=2, ensure_ascii=False)}"
            )
