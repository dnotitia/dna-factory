...
Usage:
  Check mode:
    python utils/check-GRPO-dataset.py <name_or_path> [--config <name>] [--split <split>] [--require-solution]
...
def validate_prompt_schema(dataset):
        ...
        for idx, example in enumerate(dataset):
            ...
            if is_conversational(prompt):
                if prompt[-1].get("role") != "user":
                    ...
                    return (
                        False,
                        f"Row {idx}: last prompt message role is not 'user'",
                        example,
                    )
                continue
            print(
            ...
            return (
                False,
                f"Row {idx}: 'prompt' is neither a string nor a list of dicts",
                example,
            )
    ...
    return (
        False,
        f"Dataset has neither `messages` nor `prompt` (columns: {columns})",
        None,
    )
...
def check_solution_column(dataset):
    ...
    return (
        True,
        f"'solution' column present; {missing}/{len(dataset)} rows would be excluded",
        None,
    )
...
def check_mode(dataset_name, config_name, split, require_solution):
    ...
    if require_solution:
        ...
        print(
            f"{GREEN}v {sol_message}{RESET}"
            if ok
            else f"{YELLOW}[WARNING] {sol_message}{RESET}"
        )
