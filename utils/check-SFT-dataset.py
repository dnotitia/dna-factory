...
def validate_dataset(dataset):
    ...
    for idx, example in enumerate(dataset):
        ...
        if not isinstance(messages, list) or len(messages) == 0:
            print(
                f"{YELLOW}Example data:{RESET} {json.dumps(example, indent=2, ensure_ascii=False)}"
            )
        ...
        for msg_idx, message in enumerate(messages):
            if not isinstance(message, dict):
                ...
                print(
                    f"{YELLOW}Complate data:{RESET} {json.dumps(example, indent=2, ensure_ascii=False)}"
                )
            ...
            if "content" not in message or "role" not in message:
                print(
                    f"{YELLOW}Message data:{RESET} {json.dumps(message, indent=2, ensure_ascii=False)}"
                )
                print(
                    f"{YELLOW}Complate data:{RESET} {json.dumps(example, indent=2, ensure_ascii=False)}"
                )
                return (
                    False,
                    f"Row {idx}, Message {msg_idx}: Missing 'content' or 'role'",
                    example,
                )
            ...
            if role == "user" or role == "system":
                ...
                if "thinking" in message and message["thinking"] is not None:
                    ...
                    return (
                        False,
                        f"Row {idx}, Message {msg_idx}: {role.capitalize()} message should not have 'thinking' field (unless null)",
                        example,
                    )
            elif role == "assistant":
                ...
                if "thinking" not in message:
                    ...
                    return (
                        False,
                        f"Row {idx}, Message {msg_idx}: Assistant message missing 'thinking' field",
                        example,
                    )
            else:
                print(
                ...
                return (
                    False,
                    f"Row {idx}, Message {msg_idx}: Invalid role '{role}' (must be 'system', 'user' or 'assistant')",
                    example,
                )
...
def transform_type1(example):
    ...
    if "messages" not in example:
        raise ValueError("'messages' field not found in example.")
...
def transform_type2(example):
    ...
    if "conversations" not in example:
        raise ValueError("'conversations' field not found in example.")
...
def convert_mode(source_dataset, target_dataset, conversion_type):
    ...
    if conversion_type == "type1":
        ...
        dataset = dataset.map(
            transform_type1, desc="Converting messages to thinking format (type1)"
        )
